...
export default function AdminChatPage() {
  ...
  const send = async (event) => {
    ...
    try {
      ...
      pushToast(error.response?.data?.message || t('k_0106', 'ສົ່ງຂໍ້ຄວາມບໍ່ສຳເລັດ'), 'error');
    }
  };

  return (
    <div className="space-y-6">
      <PageHeader title={t('k_0168', 'ແຊັດກັບຜູ້ເຊົ່າ')} subtitle={t('k_0053', 'ດຶງບົດສົນທະນາ 2 ທາງ ແລະ ສົ່ງຂໍ້ຄວາມ')} />
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        ...
        <div className="card flex h-[70vh] flex-col overflow-hidden">
          <div className="border-b border-slate-100 p-4"><h3 className="font-bold">{selectedTenant?.name || t('k_0154', 'ເລືອກຜູ້ເຊົ່າ')}</h3></div>
