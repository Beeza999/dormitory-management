...
export default function UserHistoryPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="ປະຫວັດການຈ່າຍ" subtitle="ບິນທີ່ຈ່າຍແລ້ວທັງໝົດ" />
          ...
          <div key={bill._id} className="card p-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">ຄົບກຳນົດ {fmtDate(bill.dueDate)}</p>
              </div>
              <p className="text-xl font-black text-emerald-600">{fmtMoney(bill.totalAmount)}</p>
            </div>
          </div>
        )) : <div className="card p-5 text-sm text-slate-500">ບໍ່ມີປະຫວັດການຈ່າຍ</div>}
      </div>
    </div>
  );
}
