...
const menus = [
  ['/user/dashboard', 'ໜ້າຫຼັກ', LayoutDashboard],
  ['/user/bills', 'ບິນ', Receipt],
  ['/user/payment', 'ຈ່າຍ', CreditCard],
  ['/user/announcements', 'ປະກາດ', Megaphone],
  ['/user/notifications', 'ແຈ້ງເຕືອນ', Bell],
  ['/user/chat', 'ແຊັດ', MessageCircle],
  ['/user/profile', 'ໂປຣໄຟລ໌', User2],
];
    ...
    <div className="min-h-screen bg-slate-100 pb-24 md:pb-8">
      <div className="mx-auto max-w-6xl p-4 md:p-6 xl:p-8">
        <div className="mb-6 flex flex-col gap-4 rounded-3xl bg-white p-4 shadow-soft md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm text-slate-500">ບີຫ້ອງເຊົ່າ</p>
            <p className="text-xl font-bold text-slate-950">ສະບາຍດີ, {user?.name || 'User'}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={logout} className="btn-outline w-full md:w-auto">ອອກຈາກລະບົບ</button>
          </div>
        </div>
        <Outlet />
      </div>
